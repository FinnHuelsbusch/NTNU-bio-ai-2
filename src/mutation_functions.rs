...
fn two_opt(genome: &Genome, problem_instance: &ProblemInstance, _config: &Config) -> Genome {
    ...
    let nurese: usize =
        nurses_with_three_or_more_patients
        ...
        for i in 0..journey.len() - 1 {
            for j in i..journey.len() {
                ...
                if i == 0 {
                    if j == journey.len() - 1 {
                        old_distance =
                            problem_instance.travel_time[0][journey[0]] +
                            problem_instance.travel_time[journey[j]][0];
                        new_distance =
                            problem_instance.travel_time[0][journey[j]] +
                    ...
                    old_distance =
                        problem_instance.travel_time[journey[i - 1]][journey[i]] +
                        problem_instance.travel_time[journey[j]][0];
                    new_distance =
                        problem_instance.travel_time[journey[i - 1]][journey[j]] +
