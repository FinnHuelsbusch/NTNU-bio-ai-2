...
class Config:
    def __init__(self, name) -> None:
        ...
        self.init_heuristic = init_heuristic[random.randint(
            0, init_heuristic.__len__()-1)]
        self.population_size = population_size[random.randint(
            0, population_size.__len__()-1)]
        self.number_of_generations = number_of_generations[random.randint(
            0, number_of_generations.__len__()-1)]
        self.parent_selection = parent_selection[random.randint(
            0, parent_selection.__len__()-1)]
        self.tournament_size_parent_selection = tournament_size_parent_selection[random.randint(
            0, tournament_size_parent_selection.__len__()-1)]
        self.tournament_probability_parent_selection = tournament_probability_parent_selection[random.randint(
            0, tournament_probability_parent_selection.__len__()-1)]
        self.elite_percentage_parent_selection = elite_percentage_parent_selection[random.randint(
            0, elite_percentage_parent_selection.__len__()-1)]
        self.cross_over_probability_partiallyMappedCrossover = cross_over_probability[random.randint(
            0, cross_over_probability.__len__()-1)]
        self.cross_over_probability_orderOneCrossover = cross_over_probability[random.randint(
            0, cross_over_probability.__len__()-1)]
        self.cross_over_probability_edgeRecombination = cross_over_probability[random.randint(
            0, cross_over_probability.__len__()-1)]
        self.mutation_probability_reassignOnePatient = mutation_probability[random.randint(
            0, mutation_probability.__len__()-1)]
        self.mutation_probability_moveWithinJourney = mutation_probability[random.randint(
            0, mutation_probability.__len__()-1)]
        self.mutation_probability_swapBetweenJourneys = mutation_probability[random.randint(
            0, mutation_probability.__len__()-1)]
        self.mutation_probability_swapWithinJourney = mutation_probability[random.randint(
            0, mutation_probability.__len__()-1)]
        self.mutation_probability_insertionHeuristic = mutation_probability[random.randint(
            0, mutation_probability.__len__()-1)]
        self.percentage_to_slice = percentage_to_slice[random.randint(
            0, percentage_to_slice.__len__()-1)]
        self.survivor_selection = survivor_selection[random.randint(
            0, survivor_selection.__len__()-1)]
        self.elite_percentage_survivor_selection = elite_percentage_survivor_selection[random.randint(
            0, elite_percentage_survivor_selection.__len__()-1)]
        self.combine_parents_and_offspring = combine_parents_and_offspring[random.randint(
            0, combine_parents_and_offspring.__len__()-1)]
        self.tournament_size_survivor_selection = tournament_size_survivor_selection[random.randint(
            0, tournament_size_survivor_selection.__len__()-1)]
        self.tournament_probability_survivor_selection = tournament_probability_survivor_selection[random.randint(
            0, tournament_probability_survivor_selection.__len__()-1)]
    ...
    def run_tests(self):
        ...
        for i in train_instance:
            ...
            result = subprocess.run(
                ["./target/release/bio-ai-2", f"./configs/{self.name}.json"], stdout=subprocess.PIPE)
        ...
        os.remove("./configs/" + self.name + ".json")
